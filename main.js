...
function buyGift(input, gifts, tickets) {
    if (Object.keys(gifts).length === 0) {
        return console.log("Wow! There are no gifts to buy.");
    }
    let answer = Number(input("Enter the number of the gift you want to get:"));
...
function addTickets(input, tickets) {
    let answer = Number(input("Enter the ticket amount: "));
...
1-Buy a gift 2-Add tickets 3-Check tickets 4-Show gifts 5-Exit the shop\n`))) {
            case 1:
                buyGift(input, gifts, tickets);
                continue;
            case 2:
                addTickets(input, tickets);
                continue;
            case 3:
                showTickets(tickets);
                continue;
